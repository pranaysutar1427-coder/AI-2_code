...
describe('IWS Regression Test', () => {
  ...
  // it('SV-5290: Verify that after user submitted reset password request user getting email with reset password link', async () => { 
  //   await terpPage.goto(`${config.url}/iws`);
  //   await terpPage.waitForSelector('input[name="username"]', { timeout: config.timeout }, { visible: true });
  //   await iwsPageObjects.clickOnForgotPassword(terpPage);
  //   await iwsPageObjects.enterUsernameAndClickSubmit(terpPage, config.terpUser3.username);
  //   await delay(2000);
  //   await expect(terpPage).toMatch('Please check your email for a reset password link.', {timeout: config.timeout});
  //   await htmlReport.attachScreenshot(terpPage, "Reset Password");
  //   await iwsPageObjects.clickBackToLogin(terpPage);
  // });

  // it('SV-7605: Verify Customize Forgot Password Reset 429 Message', async () => {
  ...
  //     try {
  //       await terpPage.waitForXPath("//div[text()='Please correct the following errors']", { timeout: 1500 }, { visible: true });
  //       await expect(terpPage).toMatchElement('li', { text: "Too many reset attempts. Please wait 30 minutes and try again." },  { timeout: config.timeout }, { visible: true });
  //       break;
  //     } catch (err) {
  ...
  //   await htmlReport.attachScreenshot(terpPage, "Throttled Password Reset Message");
  //   await iwsPageObjects.clickBackToLogin(terpPage);
  // });

  //navigating to outlook, clicking reset password will be changed after any way out is considered
  /*it('SV-5291: Verify that after user use reset password link, create new password and login to IWS', async () => { 
    
    await terpPage.goto('https://outlook.office.com/mail/inbox'); //TODO: change to diffrecnt test SMTP Server
    await expect(terpPage).toMatch('Sign in');
    await terpPage.type('input[type="email"]', config.testEmailId.username);
    await terpPage.click('input[type="submit"]');
    await terpPage.waitForNavigation();
    await expect(terpPage).toMatch('Enter password');
    await terpPage.type('input[type="password"]', config.testEmailId.password);
    await terpPage.click('input[type="submit"]');
    await terpPage.waitForNavigation();
    await expect(terpPage).toMatch('Stay signed in?');
    await terpPage.click('input[type="button"]');
    await terpPage.waitForSelector('button[type="button"]');
    const firstResetPasswordMail = await terpPage.$x('//span[text()=\'Stratus Video Password Reset\']');
    await firstResetPasswordMail[0].click();
    await terpPage.waitForSelector('i[data-icon-name="Like"]');

    await expect(terpPage).toClick('a', { text: 'Click here to reset your password.' }, {
    ...
    await expect(newPage).toMatch('Reset Password');
    await newPage.type('input[name="password"]', config.clientUser1.password);
    await newPage.type('input[name="passwordConfirm"]', config.clientUser1.password);
    await newPage.click('button[type="submit"]');

    await expect(newPage).toMatchElement(
      'p', {
    ...
    await expect(terpPage).toMatchElement('img[alt="AMN Healthcare Logo"]', {timeout:config.delay});
    const caretDownButton = await terpPage.$x('//button[@type=\'button\']//*[@data-icon=\'caret-down\']');
    await caretDownButton[0].click();
    const logoutButton = await terpPage.$x('//div[text()=\'Sign Out\']');
  ...
  it('SV-5302: Verify that user can change ringtone by selecting it from "Ringtone" dropdown and play it by clicking on "Start Ringing" button', async () => { 
    ...
    // const ringtoneDropdown = await terpPage.$x('//div[@id=\'settings--ring-tone\']');
    // await ringtoneDropdown[0].click();

    // const selectRingtone = await terpPage.$x('//div[@id=\'settings--ring-tone\']//input[1]');
    // await selectRingtone[0].click();

    // const startRingingButton = await terpPage.$x('//*[text()="Start Ringing"]');
    // await startRingingButton[0].click();
    // await htmlReport.attachScreenshot(terpPage, "Start Ring");
  ...
  it('prep terp to receive a call', async () => {
    
    await iwsPageObjects.changeUserStatus(terpPage, "Available");
    await htmlReport.attachScreenshot(terpPage, "Terp Available");
  ...
  it('login and prep terp2', async () => {
    ...
    if (isFree) {
      // go unavailable
      await expect(terp2Page).toMatchElement('div[role="tablist"]', {timeout:config.delay});
      await iwsPageObjects.changeUserStatus2(terp2Page, "Unavailable");
      await delay (2000);
  ...
  // it('SV-5420: Verify that user getting "Video Session" call notification', async () => {	

  //   await expect(terpPage).waitForSelector('span[icon="phone"]' ,{timeout: config.timeout});
  //   await helperFuncs.verifyTextXpath(terpPage,"//span[@icon='phone']//following::span[1]","Video Session");
  //   await htmlReport.attachScreenshot(terpPage, "Terp Gets Video Session Notification");
  ...
  // it('SV-7067: Verify ESC key is disabled for direct offer, transfer, and incoming call (and requeue)', async () => {
    
  //   await iwsPageObjects.pressEscapeOnCallNotification(terpPage);
  //   await expect(terpPage).waitForSelector('span[icon="phone"]' ,{timeout: config.timeout});
  //   await htmlReport.attachScreenshot(terpPage, "ESC Key Disabled");
  ...
  it('Skip Billing Questions', async () => {
   ...
   ---------it('SV-5401: Verify that user presented with "Volume", "Toggle Microphone", "Toggle Privacy", "Refresh Call", "Digital Whiteboard", "Collapse" buttons, "Volume" adjuster, "signal level" indicator, "End" button', async () => {

    await expect(terpPage).toMatchElement('button[title="Volume"]', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('button[title="Toggle Microphone"]', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('button[title="Toggle Privacy"]', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('button[title="Refresh Call"]', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('button[title="Digital Whiteboard"]', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('button[title="Collapse"]', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('input[type="range"]', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('button[title="End"]', {timeout: config.timeout});
  ...
  it('SV-5349: Verify that "Session Controls" includes "Phone", "Transfer Session", "Add Interpreter" tabs', async () => {
    await expect(terpPage).toMatchElement('#bp3-tab-title_SessionControls_phoneTab', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('#bp3-tab-title_SessionControls_transferTab', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('#bp3-tab-title_SessionControls_terpTab', {timeout: config.timeout});
  ...
  it('SV-5350: Verify that if user click on "Phone" tab user will be presented with "Add a phone participant to the session..." message, "International Calling" status, "Enter phone number" field, and "Call" button', async () => {
    await terpPage.click('#bp3-tab-title_SessionControls_phoneTab');
  ...
  it('SV-5352: Verify that if "International Calling" set to "Enabled" (Django admin config) user can make international calls', async () => {
    await delay(4000);
    await expect(terpPage).toMatchElement('span', {text: 'Disabled'}, {timeout: config.timeout});
    
  });

  it('SV-5354: Verify that "Call" button disabled if full phone number is not entered', async () => {
    await expect(terpPage).toMatchElement('button[class="bp3-button bp3-fill bp3-intent-primary"]', {timeout: config.timeout});
  ...
  it('SV-5356: Verify that if user click on "Interpreter" tab, user will be presented with "Add an interpreter to the session. You will remain in session.Do not use for ..." message, "Create New Request" button', async () => {
    await terpPage.click('#bp3-tab-title_SessionControls_terpTab');
  ...
  it('terp2 goes available', async () => {
    ...
    await htmlReport.attachScreenshot(terp2Page, "Terp2 Available");

    });

  it('terp adds party', async () => {
    await terpPage.waitForSelector('#bp3-tab-title_SessionControls_terpTab', {visible: true});
    await terpPage.click('#bp3-tab-title_SessionControls_terpTab');
  ...
  it('SV-5371: Verify that user can clear "enter digits" field by using X button', async () => {
    ...
    const clearXButton = await terpPage.$x("//button[@type='button']//span[@icon='cross']");
    await clearXButton[0].click();
    await expect(terpPage).toMatchElement('span', {text: 'Enter digits'}, {timeout: config.timeout});
  ...
  it('SV-5372: Verify if click on "Incident Report" dropdown user will be presented with "Technical" and "Operational" tabs', async () => {
    ...
    await expect(terpPage).toMatchElement('#bp3-tab-title_Incident_technicalIncidentTab', {timeout: config.timeout});
    await expect(terpPage).toMatchElement('#bp3-tab-title_Incident_operationalIncidentTab', {timeout: config.timeout});
  ...
  it('SV-5385: Verify if click on "Technical" tab user will be presented with "Issues related to session quality will be sent to CST" message, "Create New Incident" button', async () => {
    ...
    const technicalIncidentTab = await terpPage.$x("//div[@id='bp3-tab-title_Incident_technicalIncidentTab']");
  ...
  it('SV-5392: Verify if click on "Create New Incident" button user will be presented with different type of questions, "Comments" field, "Reset" button, "Submit incident" buttons', async () => {
    ...
    const createNewIncidentButton = await terpPage.$x("//div[@id='bp3-tab-panel_Incident_technicalIncidentTab']//span[text()='Create New Incident']");
  ...
  it('SV-5394: Verify if user click "Reset" button, all answered questions will be cleared', async () => {
    ...
    const commentsTextArea = await terpPage.$x("//textarea[@name='message']");
    await commentsTextArea[0].type('Test Comments');
    const resetIRButton = await terpPage.$x("//div[@id='bp3-tab-panel_Incident_technicalIncidentTab']//span[text()='Reset']");
  ...
  it('SV-5395: Verify if click on "Operational" tab user will be presented with "Issues related to session users will be sent to Account management or Language Operations." message, "Create New Incident" button', async () => {
    ...
    const operationalIncidentTab = await terpPage.$x("//div[@id='bp3-tab-title_Incident_operationalIncidentTab']");
  ...
  it('SV-5396: Verify if click on "Create New Incident" button user will be presented with "Do not submit personal health information." message, "Comments" field, "Reset" button, "Submit incident" buttons', async () => {
    ...
    const createNewIncidentButton = await terpPage.$x("//div[@id='bp3-tab-panel_Incident_operationalIncidentTab']//span[text()='Create New Incident']");
    await createNewIncidentButton[0].click();
    ...
    await questionOneDropDown[0].click();
    await terpPage.keyboard.press('Enter');
    const commentsTextArea = await terpPage.$x("//div[@id='bp3-tab-panel_Incident_operationalIncidentTab']//textarea[@name='message']");
    await commentsTextArea[0].type('Test Comments');
    await expect(terpPage).toMatchElement('span', {text: 'Reset'}, {timeout: config.timeout});
    await expect(terpPage).toMatchElement('span', {text: 'Submit Incident'}, {timeout: config.timeout});
  ...
  it('SV-5397: Verify that user can submit "Operational" incident report', async () => {
    ...
    const submitIncidentButton = await terpPage.$x("//div[@id='bp3-tab-panel_Incident_operationalIncidentTab']//button[@data-e2e='submit-incident']");
    await submitIncidentButton[0].click();
    await terpPage.waitForXPath("//div[@id='bp3-tab-panel_Incident_operationalIncidentTab']//div[@data-e2e='alert-box']", { visible: true });
  ...
  it('SV-5398: Verify if user click "Reset" button, the "Comments" field will be cleared', async () => {
    ...
    const commentsTextArea = await terpPage.$x("//div[@id='bp3-tab-panel_Incident_operationalIncidentTab']//textarea[@name='message']");
    await commentsTextArea[0].type('Test Comments');
    const resetOIButton = await terpPage.$x("//div[@id='bp3-tab-panel_Incident_operationalIncidentTab']//span[text()='Reset']");
  ...
  it('terp2 go Available', async () => {
    ...
    await terp2Page.waitForSelector('button[class="bp3-button bp3-intent-success"]', { timeout: config.timeout });
  ...
  it('SV-5366: Verify that user can make a transfer by selecting available terp from dropdown list', async () => {
    ...
    await terpPage.waitForSelector('#bp3-tab-title_SessionControls_transferTab',{ timeout: config.timeout });
    await terpPage.click('#bp3-tab-title_SessionControls_transferTab');
    const selectTransferTypeDropdown = await terpPage.$x("//div[text()='Select a transfer type']");
    await selectTransferTypeDropdown[0].click();
    ...
    const selectInterpreter = await terpPage.$x('//*[text()=\'QA-TCB-IWSTerp2 #QA-TCB-IWSTerp2\']');
  ...
  it('terp2 answers transfer call', async () => {
    ...
    await iwsPageObjects.iwsAcceptCall(terp2Page, "Transfer");
    await htmlReport.attachScreenshot(terp2Page, "Terp2 Accepts Call");
    await delay(5000);
  });
  ...
  it('SV-5361: Verify if click on "Create New Request" button, user will be presented with "Language" and "Skills" dropdown', async () => {
    ...
    await terp2Page.waitForSelector('#bp3-tab-title_SessionControls_transferTab',{ timeout: config.timeout });
    await terp2Page.click('#bp3-tab-title_SessionControls_transferTab');
  ...
  it('SV-5425: Verify that "Video Requeue" notification includes "Provider", "Enterprise", "Name", "Phone Number", "Language", "City", "State" Info, Reason"Decline", "Accept" and "X" buttons', async () => {	
    ...
    await terpPage.waitForXPath("//span[@icon='phone']", { timeout: config.timeout });
    await expect(terpPage).toMatchElement('dt', {text: 'Provider:'},{timeout: config.timeout});	
    await expect(terpPage).toMatchElement('dt', {text: 'Enterprise:'},{timeout: config.timeout});	
    await expect(terpPage).toMatchElement('dt', {text: 'Name:'},{timeout: config.timeout});	
    await expect(terpPage).toMatchElement('dt', {text: 'Phone Number:'},{timeout: config.timeout});	
    ...
    await expect(terpPage).toMatchElement('span', {text: 'Decline'},{timeout: config.timeout});	
    await expect(terpPage).toMatchElement('span', {text: 'Accept'},{timeout: config.timeout});
    await expect(terpPage).toMatchElement('span[icon="small-cross"]',{timeout: config.timeout});	
  ...
  it('terp has video', async () => {
    await expect(terpPage).toMatchElement('#session--remote-video-container video', { timeout: config.timeout });
  });
  ...
  it('SV-5321: Verify that user can\'t submit reports without answering required fields', async () => { 
    ...
    await expect(terpPage).toMatchElement({text: 'Required'}, {timeout: config.timeout});
  ...
  it('change terp2 status to break', async () => {
    await iwsPageObjects.changeUserStatus(terp2Page, "Transfer");
  ...
  it('SV-5422: Verify if user click "Decline" to call notification, status will change to "Unavailable"', async () => {
    ...
    await iwsPageObjects.iwsDeclineCall(terpPage, "Queue");
    await terpPage.waitForXPath("//span[@id='button--listbox-input--2']//span[@icon='caret-down']", { timeout: config.timeout }, { visible: true });
    await helperFuncs.verifyTextXpath(terpPage,"//span[@id='button--listbox-input--2']//span[2]","Unavailable");
  ...
  it('SV-5423: Verify if user click "X" in call notification, notification will be dismissed and status will change to "Unavailable"', async () => {
    ...
    await iwsPageObjects.iwsDeclineCallXButton(terpPage, "Queue");
    await terpPage.waitForXPath("//span[@id='button--listbox-input--2']//span[@icon='caret-down']", { timeout: config.timeout }, { visible: true });
    await helperFuncs.verifyTextXpath(terpPage,"//span[@id='button--listbox-input--2']//span[2]","Unavailable");
    await htmlReport.attachScreenshot(terpPage, "Decline Call Changes Status To Unavailable");
  ...
  it('SV-7668: Verify Changed Missed Opportunity Time', async () => {
    ...
    await htmlReport.attachScreenshot(terpPage, "30 Seconds Missed Opportunity");
    
  });

  it('cancel call from client', async () => {
    
    await page.click('button[class="btn btn-primary btn-xs"]');
    await page.waitForSelector("button[class='btn btn-success btn-xs']", { visible: true });
    
  });

  it('SV-7435: Verify missed Opportunity record in history section', async () => {
    
    await terpPage.waitForXPath("(//td[@class='dashboard--history-event'])[1]", { timeout: config.timeout }, { visible: true });
    await terpPage.waitFor(1000);
    await helperFuncs.verifyTextXpath(terpPage,"(//td[@class='dashboard--history-event'])[1]","Call Timed Out (30s)");
    await htmlReport.attachScreenshot(terpPage, "Call Timed Out In History");
